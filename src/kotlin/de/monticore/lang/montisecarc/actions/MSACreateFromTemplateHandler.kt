 ...
 */
class  MSACreateFromTemplateHandler: DefaultCreateFromTemplateHandler() {

    override fun handlesTemplate(template: FileTemplate?): Boolean {
        val templateOfType = template?.isTemplateOfType(MSAFileType.instance)
        if(templateOfType == null) {
            return false
        }
        else {
            return templateOfType
        }
    }
}